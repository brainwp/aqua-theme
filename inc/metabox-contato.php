...
add_action('admin_menu', 'contato_add_box');
   

// Adiciona o MetaBox
function contato_add_box() {
    global $meta_box;
	global $post;
	$post_id = $_GET['post'] ? $_GET['post'] : $_POST['post_ID'] ;    
	$template_file = get_post_meta($post_id,'_wp_page_template',TRUE);
	if ($template_file == 'page-contato.php'){
		add_meta_box(
			$meta_box['id'],
			$meta_box['title'], 
			'contato_show_box',
...
function contato_show_box() {
    global $meta_box, $post;
    // Use nonce for verification
    echo '<input type="hidden" name="contato_meta_box_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
	...
	add_action('save_post', 'contato_save_data');
    
	// Save data from meta box
    function contato_save_data($post_id) {
    global $meta_box;
	
    // verify nonce
    if (!wp_verify_nonce($_POST['contato_meta_box_nonce'], basename(__FILE__))) {
    return $post_id;
    }
